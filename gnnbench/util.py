...
def generate_grid_parameter_settings(search_spaces_dict):
    settings = {}
    param_name = []
    param_values = []
    for param in search_spaces_dict:
        if search_spaces_dict[param]["format"] == "range":
            raise ValueError('Cannot perform grid search on a "range" - use "values" instead')
        elif search_spaces_dict[param]["format"] == "values":
            param_name.append(param)
            settings[param] = []
            if type(search_spaces_dict[param]["values"]) is list:
                temp = search_spaces_dict[param]["values"]
            else:
                temp = search_spaces_dict[param]["values"].tolist()
            param_values.append(temp)
        else:
            raise ValueError(f"Unknown format {search_spaces_dict[param]['format']}.")
    prod = itertools.product(*param_values)
    num_different_configs = 0
    for tup in prod:
        num_different_configs += 1
        for i, config_value in enumerate(tup):
            settings[param_name[i]].append(config_value)
    return settings, num_different_configs
