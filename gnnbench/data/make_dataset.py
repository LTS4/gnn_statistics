...
def get_train_val_test_split(dataset, random_state,
                             labels,
                             train_examples_per_class=None, val_examples_per_class=None,
                             test_examples_per_class=None,
                             train_size=None, val_size=None, test_size=None, train_proportion=None):

    num_samples, num_classes = labels.shape
    remaining_indices = list(range(num_samples))

    smallest_classes = {'cora': 131,
                        'pubmed': 4103,
                        'citeseer': 115}

    if train_proportion is not None:
        train_size = int(train_proportion * num_samples)
        by_class = int(train_size / num_classes) + 1
        if dataset in smallest_classes:
            smallest_class = smallest_classes[dataset]
            if by_class <= smallest_class:
                # Enough samples to pick them equally in each class
                # Potential rounding errors by one training example
                train_examples_per_class = by_class
        else:
            raise ValueError('train_proportion should not be used on other datasets than Planetoid')
